    ...
    <div className="flex gap-2 justify-center mb-8" dir="ltr" onPaste={onPaste}>
      {digits.map((d, i) => (
        <input
          ...
          inputMode="numeric"
          autoComplete="off"
          enterKeyHint={i === digits.length - 1 ? 'done' : 'next'}
          maxLength={1}
          value={toPersianDigits(d)}
          onChange={(e) => onDigitChange(i, e.target.value)}
          onKeyDown={(e) => onKeyDown(i, e)}
          className="w-[60px] h-12 text-center text-lg rounded-lg border-2 bg-transparent text-gray-800 border-[#E5E7EB] focus:outline-none focus:ring-2 focus:ring-[#0095DA] focus:border-transparent"
          dir="ltr"
        />
      ))}
    </div>
